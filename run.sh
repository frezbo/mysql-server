...
rm -f "$import_file"
elif [ ! -z "$MYSQL_ROOT_PASS" -o ! -z "$MYSQL_DB_NAME" -o ! -z "$MYSQL_DB_USER" -o ! -z "$MYSQL_DB_PASS" ]; then
	echo "Arguments should not be provided while running the container as daemon"
	exit 1
else
	mkdir -p /run/mysqld
